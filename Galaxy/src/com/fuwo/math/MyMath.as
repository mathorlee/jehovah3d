...
//			var p3p2:Point = p2.subtract(p3);
		...
		public static function analysisFileExtentionFromURL(url:String):String
		{
			...
			if(realFileName.indexOf(".") == -1) //若文件不存在后缀名，返回空。
				return null;
			fileExtention = analysisFileExtentionFromFileName(realFileName);
			if(fileExtention == "")
				fileExtention = null;
			if(paramURL)
			{
				...
				if(obj.name && obj.name != "")
					return analysisFileExtentionFromFileName(obj.name);
			}
			return fileExtention;
		}
		
		public static function analysisFileExtentionFromFileName(fileName:String):String
		{
		 ...
		 * @param arr0
		 * @param arr1
		 * @param mergeToFirstArray
		 * @return 
		 * 
		 */		
		public static function mergeTwoArray(arr0:Object, arr1:Object, mergeToFirstArray:Boolean):Object
		{
			var i:int;
			if(mergeToFirstArray)
			{
		...
		public static function numberArrayToNumberVector(arr:Object):Vector.<Number>
		{
		...
		/**
		 * [*, *]或Vector<*>转化为[Object, Object, ...]
		 * @param source
		 * @return 
		 * 
		 */		
		public static function arrayOrVectorToObjectArray(source:Object):Array
		{
			var i:int;
			var ret:Array = [];
			...
			{
				var item:Object = {};
				if(source[i].hasOwnProperty("x"))
					item.x = source[i].x;
				if(source[i].hasOwnProperty("y"))
					item.y = source[i].y;
				if(source[i].hasOwnProperty("z"))
					item.z = source[i].z;
		...
		public static function objectArrayToPointVector(points:Array):Vector.<Point>
		{
			var ret:Vector.<Point> = new Vector.<Point>();
			var i:int;
			for(i = 0; i < points.length; i ++)
				ret.push(new Point(points[i].x, points[i].y));
			return ret;
		}
		...
		/**
		 * 输入一个点的list，返回boundary。包含minXYZ和maxXYZ
		 * @param points
		 * @return 
		 * 
		 */		
		public static function findBoundaryOfPoints(points:Object):Object
		{
			var ret:Object = {
			...
			for each(var p:Object in points)
			{
				...
				{
					ret.minX = Math.min(ret.minX, p.x);
					ret.maxX = Math.max(ret.maxX, p.x);
				}
				if(p.hasOwnProperty("y"))
				{
					ret.minY = Math.min(ret.minY, p.y);
					ret.maxY = Math.max(ret.maxY, p.y);
				}
				if(p.hasOwnProperty("z"))
				{
					ret.minZ = Math.min(ret.minZ, p.z);
					ret.maxZ = Math.max(ret.maxZ, p.z);
				}
			}
			return ret;
		}
		...
		/**
		 * 旋转一个bitmapData，返回新的bitmapData
		 * @param angle
		 * @param source
		 * @return 
		 * 
		 */		
		public static function rotateBitmapData(angle:Number, source:BitmapData):BitmapData
		{
			...
			var points:Vector.<Point> = Vector.<Point>([
				...
				matrix.transformPoint(new Point(source.width, 0)), 
				matrix.transformPoint(new Point(0, source.height)), 
				matrix.transformPoint(new Point(source.width, source.height))
			]);
			var boundary:Object = MyMath.findBoundaryOfPoints(points);
			matrix.translate(boundary.minX < 0 ? -boundary.minX : 0, boundary.minY < 0 ? -boundary.minY : 0);
			
			var ret:BitmapData = new BitmapData(boundary.maxX - boundary.minX, boundary.maxY - boundary.minY, source.transparent);
			ret.draw(source, matrix, null, null, null, true);

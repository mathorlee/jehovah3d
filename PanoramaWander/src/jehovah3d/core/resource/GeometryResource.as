package jehovah3d.core.resource
{
	...
	public class GeometryResource extends Resource
	{
		...
		private var _smoothingGroups:Vector.<uint>;
...
//			trace("geometry.upload");
		 ...
		 * 
		 */		
		public function calculateBounding():void
		{
			_bounding = new Bounding();
			var i:uint;
			for(i = 0; i < _coordinateData.length / 3; i ++)
			{
				if(_coordinateData[3 * i] > _bounding.maxX)
					_bounding.maxX = _coordinateData[3 * i];
				if(_coordinateData[3 * i] < _bounding.minX)
					_bounding.minX = _coordinateData[3 * i];
				if(_coordinateData[3 * i + 1] > _bounding.maxY)
					_bounding.maxY = _coordinateData[3 * i + 1];
				if(_coordinateData[3 * i + 1] < _bounding.minY)
					_bounding.minY = _coordinateData[3 * i + 1];
				if(_coordinateData[3 * i + 2] > _bounding.maxZ)
					_bounding.maxZ = _coordinateData[3 * i + 2];
				if(_coordinateData[3 * i + 2] < _bounding.minZ)
					_bounding.minZ = _coordinateData[3 * i + 2];
			}
			_bounding.calculateDimension();
		}
		public function get boundingPoints():Vector.<Vector3D>
		{
			var points:Vector.<Vector3D> = new Vector.<Vector3D>();
			points.push(
				new Vector3D(_bounding.maxX, _bounding.maxY, 0), 
				new Vector3D(_bounding.minX, _bounding.maxY, 0), 
				new Vector3D(_bounding.minX, _bounding.maxY, 0), 
				new Vector3D(_bounding.minX, _bounding.minY, 0), 
				new Vector3D(_bounding.minX, _bounding.minY, 0), 
				new Vector3D(_bounding.maxX, _bounding.minY, 0), 
				new Vector3D(_bounding.maxX, _bounding.minY, 0), 
				new Vector3D(_bounding.maxX, _bounding.maxY, 0), 
				
				new Vector3D(_bounding.maxX, _bounding.maxY, _bounding.maxZ), 
				new Vector3D(_bounding.minX, _bounding.maxY, _bounding.maxZ), 
				new Vector3D(_bounding.minX, _bounding.maxY, _bounding.maxZ), 
				new Vector3D(_bounding.minX, _bounding.minY, _bounding.maxZ), 
				new Vector3D(_bounding.minX, _bounding.minY, _bounding.maxZ), 
				new Vector3D(_bounding.maxX, _bounding.minY, _bounding.maxZ), 
				new Vector3D(_bounding.maxX, _bounding.minY, _bounding.maxZ), 
				new Vector3D(_bounding.maxX, _bounding.maxY, _bounding.maxZ), 
		...
		public function calculateIntersectionDist(ray:Ray):Number
		{
			...
			for(i = 0; i < _indexData.length / 3; i ++)
			{
				...
				tmp = MousePickManager.rayRectFaceIntersect(ray, face, true);
				if(tmp)
				{
					tmpDist = tmp.subtract(ray.p0).length;
					if(dist)
					{
						if(tmpDist < dist)
							dist = tmpDist;
		...
		public function set smoothingGroups(val:Vector.<uint>):void
		{
			_smoothingGroups = val.slice();
		}

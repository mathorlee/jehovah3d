package jehovah3d.core
{
	...
	public class Camera3D extends Object3D
	{
			...
			for(i = 0; i < Jehovah.lights.length; i ++)
				if(Jehovah.lights[i].useShadow)
				{
					...
					if(!Jehovah.lights[i].depthTexture)
						Jehovah.lights[i].depthTexture = Jehovah.context3D.createTexture(Jehovah.lights[i].shadowmappingsize, Jehovah.lights[i].shadowmappingsize, Context3DTextureFormat.BGRA, true, 0);
		 ...
		 * 
		 */		
		private function updateMatrices(viewObj:Object3D, projectMatrix:Matrix3D):void
		{
...
//			Jehovah.scene.localToCameraMatrix.append(viewObj.inverseMatrix);
		...
		private function renderAmbientAndReflectionTexture():void
		{
			if(!ambientAndReflectionTexture)
				ambientAndReflectionTexture = Jehovah.context3D.createTexture(1024, 1024, Context3DTextureFormat.BGRA, true, 0);
		...
		private function renderDiffuseAndSpecular():void
		{
			...
			for(i = 0; i < Jehovah.lights.length; i ++)
			{
				...
				if(!Jehovah.lights[i].diffuseAndSpecularTexture)
					Jehovah.lights[i].diffuseAndSpecularTexture = Jehovah.context3D.createTexture(1024, 1024, Context3DTextureFormat.BGRA, true, 0);
		...
		{
			if(!depthTexture)
				depthTexture = Jehovah.context3D.createTexture(1024, 1024, Context3DTextureFormat.BGRA, true, 0);
			if(!normalTexture)
				normalTexture = Jehovah.context3D.createTexture(1024, 1024, Context3DTextureFormat.BGRA, true, 0);
			...
			if(!ssaoTexture)
				ssaoTexture = Jehovah.context3D.createTexture(1024, 1024, Context3DTextureFormat.BGRA, true, 0);

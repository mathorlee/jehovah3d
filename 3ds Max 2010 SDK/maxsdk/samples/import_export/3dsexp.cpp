...
#define SUPER_3DS_EXPORT_CLASS_ID Class_ID(0x17be3067, 0x62b05236)
...
extern "C" DWORD __stdcall packfile  (char *filein, char *fileout);
...
class _3DSClassDesc:public ClassDesc {
	...
	Class_ID		ClassID() { return SUPER_3DS_EXPORT_CLASS_ID; }
...
_3DSExport::ShortDesc() {			// Short ASCII description (i.e. "Targa")
	...
	return _T("FUWO.COM 3D Model Export");
	}
...
//gdf convert to 50.3 filename
#define OFFSET2(e) ((e < 50 && e != -1) ? e : 50)
static void FiftyDotThreeName(CStr& name)
{
	CStr fExt;
    CStr fName;
    int  oExt;
    if (name.Length() > 54) {
        oExt = name.last('.');
        fName.Resize(OFFSET2(oExt));
        strncpy(fName.data(), name.data(), OFFSET2(oExt));
        if (oExt != -1) {
            fExt.Resize(4);
            strncpy(fExt.data(), name.data() + oExt, 4);
        }
        ...
        name.Resize(fName.Length() > 54 ? 54 : fName.Length());
        strcpy(name.data(), fName.data());
    }
}
...
static void ConvertBitmapTexToMapping(BitmapTex *bmt, Mapping *mp)
{
	TSTR filename;
	...
	FiftyDotThreeName(name);

	// gdf convert to Uppercase
    //name.toUpper();

	strncpy(mp->map.name,name.data(),54);
...
static void ConvertVrayMtlToSMtl(SMtl *s, Mtl *m)
{
	IParamBlock2 *pblockBasic=m->GetParamBlockByID(VRayMtlParameters::vrayMtl_basic_id);
	IParamBlock2 *pblockMaps=m->GetParamBlockByID(VRayMtlParameters::vrayMtl_maps_id);
	IParamBlock2 *pblockBRDF=m->GetParamBlockByID(VRayMtlParameters::vrayMtl_BRDF_id);
	IParamBlock2 *pblockOptions=m->GetParamBlockByID(VRayMtlParameters::vrayMtl_options_id);
	
	//basic block
	Color diffuse_color = pblockBasic->GetColor(VRayMtlParameters::pb_diffuse_color);
	float diffuse_roughness = pblockBasic->GetFloat(VRayMtlParameters::pb_diffuse_roughness);
	
	Color reflect_color = pblockBasic->GetColor(VRayMtlParameters::pb_reflect_color);
	float hilight_glossiness = pblockBasic->GetFloat(VRayMtlParameters::pb_hilight_glossiness);
	float reflect_glossiness = pblockBasic->GetFloat(VRayMtlParameters::pb_reflect_glossiness);
	int hilight_glossiness_lock = pblockBasic->GetInt(VRayMtlParameters::pb_reflect_glossiness_lock);

	Color refract_color = pblockBasic->GetColor(VRayMtlParameters::pb_refract_color);
	float refract_glossiness = pblockBasic->GetFloat(VRayMtlParameters::pb_refract_glossiness);
	...
	map_on = pblockMaps->GetInt(VRAYMTL_TEXMAP_ON_PARAMID(SUBTEXNO_DIFFUSE));
	map_mult = pblockMaps->GetFloat(VRAYMTL_TEXMAP_MULT_PARAMID(SUBTEXNO_DIFFUSE));
	if(map_on && map_mult > 0)
	{
		diffuse_texture = pblockMaps->GetTexmap(VRAYMTL_TEXMAP_PARAMID(SUBTEXNO_DIFFUSE));
		diffuse_mult = map_mult;
	}
	
	//maps::reflect map
	Texmap *reflect_texture = NULL;
	float reflect_mult = 1.0;
	map_on = pblockMaps->GetInt(VRAYMTL_TEXMAP_ON_PARAMID(SUBTEXNO_REFLECT));
	map_mult = pblockMaps->GetFloat(VRAYMTL_TEXMAP_MULT_PARAMID(SUBTEXNO_REFLECT));
	if(map_on && map_mult > 0)
	{
		reflect_texture = pblockMaps->GetTexmap(VRAYMTL_TEXMAP_PARAMID(SUBTEXNO_REFLECT));
		reflect_mult = map_mult;
	}

	//maps::bump map
	Texmap *bump_texture = NULL;
	float bump_mult = 1.0;
	map_on = pblockMaps->GetInt(VRAYMTL_TEXMAP_ON_PARAMID(SUBTEXNO_BUMP));
	map_mult = pblockMaps->GetFloat(VRAYMTL_TEXMAP_MULT_PARAMID(SUBTEXNO_BUMP));
	if(map_on && map_mult > 0)
	{
		bump_texture = bump_texture = pblockMaps->GetTexmap(VRAYMTL_TEXMAP_PARAMID(SUBTEXNO_BUMP));
		bump_mult = map_mult;
	}
	
	//brdf block
	int brdf_type = pblockBRDF->GetInt(VRayMtlParameters::pb_brdf_type);

	//options block
	int double_sided = pblockOptions->GetInt(VRayMtlParameters::pb_doubleSided);

	//convert
	s->material_type = 1;
	if(double_sided)
		s->flags |= MF_TWOSIDE;
	s->diff = C24(diffuse_color);
	s->vraymtl_reflect_color = C24(reflect_color);
	s->vraymtl_hilight_glossiness = short(hilight_glossiness * 100);
	s->vraymtl_reflect_glossiness = short(reflect_glossiness * 100);
	s->vraymtl_hilight_glossiness_lock = short(hilight_glossiness_lock);
	Mapping *mp;
	if(diffuse_texture)
	{
		if(diffuse_texture->ClassID() == Class_ID(BMTEX_CLASS_ID, 0))
		{
			BitmapTex *bmt = (BitmapTex*)diffuse_texture;
			s->map[0] = mp = NewMapping(0, 0);
			s->map[0]->use = 1;
			ConvertBitmapTexToMapping(bmt, mp);
		}
	}
}

/*
material_type(0: stdmtl, 1: vraymtl)
material_name
material_two_sided
...
static void ConvertMaxMtlToSMtl(SMtl *s, Mtl *m) {
	if(m->ClassID() == VRAYMTL_CLASS_ID)
	{
		ConvertVrayMtlToSMtl(s, m);
		return ;
	}
...
//		case OBJ_PROCEDURAL:	/* Special null chunk */
		...
		case N_TRI_OBJECT: {
			...
			if(mesh.numVerts > 0) {
				...
				BOOL hasUV = mesh.getNumTVerts() > 0 ? TRUE : FALSE;

				ConvertTo3DSFaces_NoUV(oe, verts, faces);
                ...
                if(mesh.getNumVerts()) {
                    ...
                    cnt  = (DWORD) verts.Count();
                    fwrite(&cnt, 1, sizeof(DWORD), wstream);
                    for (int i = 0; i < cnt; i++) {
                        pt = verts[i].pt;
						pt[0] *= xxform;
						//pt = pt * mat; //导出local点
					...
					if(hasUV){
                        ...
                        fwrite(&cnt, 1, sizeof(DWORD), wstream);
                        for (int i = 0; i < cnt; i++) {
							fwrite(&mesh.tVerts[i].x, sizeof(float), 1, wstream);
							fwrite(&mesh.tVerts[i].y, sizeof(float), 1, wstream);
                        }
                        WriteChunkEnd(cPtr, cBegin);
					}
				...
				if(mesh.getNumFaces() && hasUV)
				{
					cTag = UVFACE_ARRAY;
					WriteChunkBegin(cTag, cPtr, cBegin);
					cnt  = (DWORD) faces.Count();
					fwrite(&cnt, sizeof(DWORD), 1, wstream);
					for(int i = 0; i < cnt; i ++)
					{
						fwrite(&faces[i].uvNum[0], sizeof(DWORD), 1, wstream);
						fwrite(&faces[i].uvNum[1], sizeof(DWORD), 1, wstream);
						fwrite(&faces[i].uvNum[2], sizeof(DWORD), 1, wstream);
					}
					WriteChunkEnd(cPtr, cBegin);
				}
                ...
                if(mesh.getNumFaces()) {
                    ...
                    for (int i = 0; i < cnt; i++) {
                        ...
                        if (face.smGroup)
                            smoothed = TRUE;
                        fwrite(&faces[i].vNum[0], sizeof(DWORD), 1, wstream);
                        fwrite(&faces[i].vNum[1], sizeof(DWORD), 1, wstream);
                        fwrite(&faces[i].vNum[2], sizeof(DWORD), 1, wstream);
						UWORD wrtFlags = 0;
...
_3DSExport::DoExport(const TCHAR *filename,ExpInterface *ei,Interface *gi, BOOL suppressPrompts, DWORD options) {
	...
	char fileOut[1000];
	strncpy(fileOut, filename, strlen(filename) - 6);
	fileOut[strlen(filename) - 6] = '\0';
	strcat(fileOut, "F3D");
	packfile((char*)filename,fileOut);
	remove(filename);
...
void ConvertTo3DSFaces_NoUV(ObjectEntry* oe, Tab<Vert3ds>& verts, Tab<Face3ds>& faces)
{
	Face3ds tmpFace;
    Vert3ds tmpVert;
    Mesh mesh = oe->tri->GetMesh();
	...
	vector<vector<int>> brothers(mesh.getNumVerts());
	...
	for (int i = 0; i < mesh.getNumFaces(); i++) {
        ...
        for (int k = 0; k < 3; k++) {
			...
			if(hasUV)
				tmpFace.uvNum[k] = mesh.tvFace[i].t[k];
        }
        ...
        for (int j = 0; j < 3; j++) {
            ...
            tmpVert.pt    = vert; //tmpVert is vertex to be checked.
			tmpVert.smoothingGroup = face.smGroup;

			DWORD index = face.v[j];
            if(vWritten[index]) //verts[index] already exist.
			{
				if(verts[index].smoothingGroup != tmpVert.smoothingGroup) //verts[index].uv != tmpVert.uv
				{
					BOOL find = FALSE;
					for(int k = 0; k < brothers[index].size(); k ++)
					{
						if(verts[brothers[index][k]].smoothingGroup == tmpVert.smoothingGroup)
						{
							tmpFace.vNum[j] = brothers[index][k];
							find = TRUE;
							break;
						}
					}
					...
					if(!find)
					{
						brothers[index].push_back(brothers.size());
						vector<int> bro;
...
void ConvertTo3DSFaces_HasUV(ObjectEntry* oe, Tab<Vert3ds>& verts, Tab<Face3ds>& faces)
{
	...
	for (int i = 0; i < mesh.getNumFaces(); i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if(vWritten[index]) //verts[index] already exist.
			{
				if(!UVVertEqual(verts[index].tv, tmpVert.tv) || verts[index].smoothingGroup != tmpVert.smoothingGroup) //verts[index].uv != tmpVert.uv
				{
					BOOL find = FALSE;
					for(int k = 0; k < brothers[index].size(); k ++)
					{
						if(UVVertEqual(verts[brothers[index][k]].tv, tmpVert.tv) && verts[brothers[index][k]].smoothingGroup == tmpVert.smoothingGroup)
						{
							tmpFace.vNum[j] = brothers[index][k];
							find = TRUE;
							break;

package jehovah3d.util
{
	...
	public class MeshMaker
	{
		 ...
		 * 
		 */		
		public static function generateSphereGeometrySplitUV(coordinateData:Vector.<Number>, diffuseUVData:Vector.<Number>, indexData:Vector.<uint>, radius:Number, sectionSubdivision:int, matrix:Matrix3D):void
		{
			var n:int = sectionSubdivision;
			var m:int = n / 2;
			...
			for (i = 0; i <= n; i ++)
			{
				...
				for (j = 0; j <=m ; j ++)
				{
					...
					coordinateData.push(Math.cos(beta) * Math.cos(alpha) * radius, Math.cos(beta) * Math.sin(alpha) * radius, Math.sin(beta) * radius);
					diffuseUVData.push(1 - i / n, 0.5 - beta / Math.PI);
				}
			}

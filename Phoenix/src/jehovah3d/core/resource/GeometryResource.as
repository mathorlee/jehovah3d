...
//		private var _bounding:Bounding;
		 ...
		 * 
		 */		
		public function calculateIntersection(ray:Ray3D, calculateUV:Boolean = false):RayGeometryIntersection
		{
			var i:int;
			...
			for(i = 0; i < _indexData.length / 3; isQuadGeometry ? i += 2 : i ++)
			{
				...
				tmp = MyMath.rayRectFaceIntersect(ray, face, !isQuadGeometry);
				if(tmp)
				{
					tmpDist = tmp.subtract(ray.p0).length / ray.dir.length;
					if (!dist || (dist && tmpDist < dist))
					{
						dist = tmpDist;
						normal = v2.subtract(v1).crossProduct(v3.subtract(v1));
						normal.normalize();
						if (calculateUV && _diffuseUVData)
						{
//							trace("calculateUV");
							var uvs:Vector.<Point> = Vector.<Point>([
								new Point(_diffuseUVData[index1 * 2], _diffuseUVData[index1 * 2 + 1]), 
								new Point(_diffuseUVData[index2 * 2], _diffuseUVData[index2 * 2 + 1]), 
								new Point(_diffuseUVData[index3 * 2], _diffuseUVData[index3 * 2 + 1])
							]);
							uv = MyMath.calculateUV(
								Vector.<Vector3D>([v1, v2, v3]), 
								uvs, 
								new Vector3D(ray.dir.x * dist + ray.p0.x, ray.dir.y * dist + ray.p0.y, ray.dir.z * dist + ray.p0.z)
							);
						}
					}
				}
			...
			if (dist)
				return new RayGeometryIntersection(dist, new Vector3D(ray.dir.x * dist + ray.p0.x, ray.dir.y * dist + ray.p0.y, ray.dir.z * dist + ray.p0.z), normal, uv);
			return null;
		}

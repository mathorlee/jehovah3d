package jehovah3d.core.material
{
	...
	public class DiffuseMtl
	{
		...
		//alpha.
		private var _alpha:Number = 1.0; //透明度
		
		private var _doubleSided:Boolean = false; //是否是双面材质
		...
		{
			if(_diffuseMapResource && !_diffuseMapResource.isInObjectPool)
				_diffuseMapResource.dispose();
			if(_reflectionMapResource && !_reflectionMapResource.isInObjectPool)
				_reflectionMapResource.dispose();
			if(_specularMapResource && !_specularMapResource.isInObjectPool)
				_specularMapResource.dispose();
			if(_bumpMapResource && !_bumpMapResource.isInObjectPool)
				_bumpMapResource.dispose();
			if(_opacityMapResource && !_opacityMapResource.isInObjectPool)
		...
		public function set reflectionMapResource(val:TextureResourceBase):void
		{
			if(_reflectionMapResource != val)
			{
				...
				if (!_reflectionUVMatrix)
					_reflectionUVMatrix = new Matrix();
			}
		}

package jehovah3d.core.mesh.wireframe
{
	...
	public class WireFrame extends Mesh
	{
		...
		override public function updateHierarchyMatrix_CameraFinal():void
		{
			...
			for(i = 0; i < _lineCount; i ++) //线的起点终点，如果有一个在视椎外（nearClipping），则砍断它。
			{
				...
				if(s1 * s2 < 0)
				{
					dir = v2.subtract(v1);
					dir.normalize();
					ray = new Ray3D(v1, dir);
					intersect = MyMath.rayPlaneIntersect(ray, plane);
					if(s1 > 0)
		...
		override public function mousePick(ray:Ray3D, mousePoint:Point = null):void
		{
			...
			var rayCopy:Ray3D = new Ray3D(Jehovah.camera.inverseMatrix.transformVector(ray.p0), Jehovah.camera.inverseMatrix.deltaTransformVector(ray.dir));
			var i:int;
			var dist:Number;
			var tmpDist:Number;
			...
			for(i = 0; i < _lineCount; i ++)
			{
				...
				intersect = MyMath.rayRectFaceIntersect(rayCopy, rectface, false, false);
				if(intersect)
				{
			...
			if(dist)
			{
				var mpd:MousePickData = new MousePickData(dist);
				mpd.attachMesh = this;

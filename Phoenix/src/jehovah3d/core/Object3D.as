package jehovah3d.core
{
	...
	public class Object3D extends EventDispatcher
	{
		...
		public var renderPriority:int = RenderPriority.OPAQUE; //渲染优先级. 小的先渲染, 大的后渲染
		/**
		...
		public var focusBorderColor:uint = 0xFFFFFF;
		 ...
		 * 
		 */		
		public function updateMatrixAfterPositionChange():void
		{
			if (useParentMatrix) return ;
			if (!_parent) return ;
			
			if (_transformChanged)
				composeTransform();
			_localToGlobalMatrix.copyFrom(_matrix);
			_localToGlobalMatrix.append(_parent._localToGlobalMatrix);
			_globalToLocalMatrix.copyFrom(_parent._globalToLocalMatrix);
			_globalToLocalMatrix.append(_inverseMatrix);
			
			_localToCameraMatrix.copyFrom(_matrix);
			_localToCameraMatrix.append(_parent._localToCameraMatrix);
			_finalMatrix.copyFrom(_matrix);
			_finalMatrix.append(_parent._finalMatrix);
		}
		 ...
		 * 
		 */		
		public function mousePick(ray:Ray3D, mousePoint:Point = null):void
		{
			if(!mouseEnabled || !visible || culled)
				return ;
			...
			if(_doppergangerGeometry)
			{
				...
				if(intersect)
				{
					...
					var mpd:MousePickData = new MousePickData(intersect.dist, position, intersect.normal);
		 ...
		 * 
		 */		
		public function mousePickChildren(ray:Ray3D, mousePoint:Point = null):void
		{
			var i:int;
			for(i = 0; i < _childs.length; i ++)
		...
		{
			var oldRay:Ray3D = Jehovah.calculateRay(oldPoint).transform(globalToLocalMatrix);
			var newRay:Ray3D = Jehovah.calculateRay(newPoint).transform(globalToLocalMatrix);
			var p0:Object = MyMath.rayPlaneIntersect(oldRay, plane);
			var p1:Object = MyMath.rayPlaneIntersect(newRay, plane);
			if(p0 && p1)
		 ...
		 * 
		 */		
		public function routeBlocked(ray:Ray3D):Boolean
		{
			if(!_participateInCollisionDetection || !visible)
				return false;
		 ...
		 * 
		 */		
		public function routeBlockedByChildren(ray:Ray3D):Boolean
		{
			var i:int;
			for(i = 0; i < _childs.length; i ++)
		 ...
		 * 
		 */		
		public function collectRenderList(renderList:Vector.<Object3D>):void
		{
			if(!visible || culled)
				return ;
			collectChildrenRenderList(renderList);
		}
		public function collectChildrenRenderList(renderList:Vector.<Object3D>):void
		{
			var i:int;
			for(i = 0; i < _childs.length; i ++)
				_childs[i].collectRenderList(renderList);
		}
		 ...
		 * 
		 */		
		public function removeChild(child:Object3D, disposeChild:Boolean = true):void
		{
...
//			trace(this + "composeTransform");
		...
		public function set isSelected(value:Boolean):void
		{
			if(_isSelected != value)
			{
				...
				if(_isSelected)
				{
					if(!_selectWF)
					{
						_selectWF = new WireFrame(bounding.selectWFPoints, focusBorderColor,2);
		...
		public function autoUpdateRenderPriority():void
		{
		...
		{
			var vect3d:Vector3D = new Vector3D(x, y, z);
			return "[Object3D:" + name + "]"+vect3d;
		}
		
		public function outputHierarcy(level:int):void
		{
			var space:String = "";

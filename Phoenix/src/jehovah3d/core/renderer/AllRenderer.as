package jehovah3d.core.renderer
{
	...
	public class AllRenderer extends Renderer
	{
		...
		private function myCollectResource():void
		{
			...
			if(mesh.mtl.useDiffuseMapChannel)
			{
				...
				if (mesh.mtl.diffuseUVMatrix)
				{
					...
					fragmentConstants.push(mesh.mtl.diffuseUVMatrix.a, mesh.mtl.diffuseUVMatrix.c, mesh.mtl.diffuseUVMatrix.tx, 1);
					fragmentConstants.push(mesh.mtl.diffuseUVMatrix.b, mesh.mtl.diffuseUVMatrix.d, mesh.mtl.diffuseUVMatrix.ty, 1);
		...
		override public function collectResource():void
		{
			...
			assetsDict[C_FINALMATRIX] = currentVC();
			MyMath.mergeTwoArray(vertexConstants, MyMath.matrix3DToProgram3DConstants(mesh.finalMatrix), true);
			...
			MyMath.mergeTwoArray(fragmentConstants, MyMath.matrix3DToProgram3DConstants(mesh.localToGlobalMatrix), true);
		...
		private function generateFragmentShaderForSimpleMtl(assetsDict:Dictionary):String
		{
			...
			{
				if (assetsDict[DIFFUSE_UV_MATRIX]) 
				{
					//uv offset没有生效，待定 
					ret += agal("mov", "ft0", "xy", assetsDict[V_UVW], "xy");
					ret += agal("mov", "ft0", "z", assetsDict[DIFFUSE_UV_MATRIX], "w");
					ret += agal("m33", "ft0", "xyz", "ft0", "xyz", assetsDict[DIFFUSE_UV_MATRIX], null);
					
//					ret += agal("m33", "ft0", "xyz", assetsDict[V_UVW], "xyz", assetsDict[DIFFUSE_UV_MATRIX], null);
					ret += agal("tex", "oc", null, "ft0", "xy", assetsDict[FS_DIFFUSE] + texParameter("2d"), null);
				}
				else

		...
		public function splitUV(objVO:ObjectVO):void
		{
			...
			for(i = 0; i < objVO.numFaces; i ++)
			{
				for(j = 0; j < 3; j ++)
				{
					...
					if(visited[vi])
					{
						if(!MyMath.isNumberEqual(newUVs[vi * 2 + 0], objVO.uvs[uvi * 2 + 0]) || 
							!MyMath.isNumberEqual(newUVs[vi * 2 + 1], objVO.uvs[uvi * 2 + 1]))
						{
							var find:Boolean = false;
							for(k = 0; k < brothers[vi].length; k ++)
							{
								if(MyMath.isNumberEqual(newUVs[brothers[vi][k] * 2 + 0], objVO.uvs[uvi * 2 + 0]) && 
									MyMath.isNumberEqual(newUVs[brothers[vi][k] * 2 + 1], objVO.uvs[uvi * 2 + 1]))
								{
